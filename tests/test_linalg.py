...
def test_syev(width, dtype) :
    ...
    expected_vals, expected_vecs = np.linalg.eig(A)

    expected_vals = sorted(list(expected_vals))

    got_vals = ein.utils.create_tensor("Eigenvalues", [width], dtype=ein.utils.remove_complex(dtype))
    ...
    if dtype == np.float32 or dtype == np.complex64 :
        for exp, res in zip(expected_vals, got_vals) :
            assert(exp == pytest.approx(res, 1e-4))
    else :
        for exp, res in zip(expected_vals, got_vals) :
