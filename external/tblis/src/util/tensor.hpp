...
bool are_congruent_along(const marray_view<const T>& A,
                         const marray_view<const T>& B, unsigned dim)
{
                           ...
                           const stride_vector& stride_B)
{
    ...
    index_iterator<1, 1> it_A(len_Ar, stride_Ar);
    index_iterator<1, 1> it_B(len_Br, stride_Br);
...
template <typename T>
void matricize(marray_view<const T>  A,
...
template <typename T>
void matricize(marray_view<T>  A,
