...
void mult_blis(const communicator& comm, const config& cfg,
               T alpha, bool conj_A, dpd_marray_view<const T> A,
               dim_vector idx_A_AB,
               dim_vector idx_A_AC,
                        bool conj_B, dpd_marray_view<const T> B,
               dim_vector idx_B_AB,
               T beta_, bool conj_C_, dpd_marray_view<      T> C,
               dim_vector idx_C_AC)
{
...
void mult_blis(const communicator& comm, const config& cfg,
               ...
               dim_vector idx_A_AC,
                        bool conj_B, dpd_marray_view<const T> B,
               dim_vector idx_B_BC,
               T beta,  bool conj_C, dpd_marray_view<      T> C,
               dim_vector idx_C_AC,
               dim_vector idx_C_BC)
{
    unsigned nirrep = A.num_irreps();
    ...
    for (unsigned irrep_ABC = 0;irrep_ABC < nirrep;irrep_ABC++)
    {
        for (unsigned irrep_AB = 0;irrep_AB < nirrep;irrep_AB++)
        {
            ...
            while (irrep_it_ABC.next())
            {
                ...
                index_iterator<1,0> it_ABC(len_ABC);
