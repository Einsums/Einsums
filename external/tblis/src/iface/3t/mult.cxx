...
void mult(const communicator& comm,
          T alpha, dpd_marray_view<const T> A, const label_type* idx_A_,
                   dpd_marray_view<const T> B, const label_type* idx_B_,
          T  beta, dpd_marray_view<      T> C, const label_type* idx_C_)
{
...
template void mult(const communicator& comm, \
                   T alpha, dpd_marray_view<const T> A, const label_type* idx_A, \
                            dpd_marray_view<const T> B, const label_type* idx_B, \
                   T  beta, dpd_marray_view<      T> C, const label_type* idx_C);
#include "configs/foreach_type.h"

template <typename T>
void mult(const communicator& comm,
          T alpha, indexed_marray_view<const T> A, const label_type* idx_A_,
                   indexed_marray_view<const T> B, const label_type* idx_B_,
          T  beta, indexed_marray_view<      T> C, const label_type* idx_C_)
{
    unsigned ndim_A = A.dimension();
    unsigned ndim_B = B.dimension();
    unsigned ndim_C = C.dimension();
...
template void mult(const communicator& comm, \
                   T alpha, indexed_marray_view<const T> A, const label_type* idx_A, \
                            indexed_marray_view<const T> B, const label_type* idx_B, \
                   T  beta, indexed_marray_view<      T> C, const label_type* idx_C);
#include "configs/foreach_type.h"

template <typename T>
void mult(const communicator& comm,
          T alpha, indexed_dpd_marray_view<const T> A, const label_type* idx_A_,
                   indexed_dpd_marray_view<const T> B, const label_type* idx_B_,
          T  beta, indexed_dpd_marray_view<      T> C, const label_type* idx_C_)
{
    unsigned nirrep = A.num_irreps();
    TBLIS_ASSERT(B.num_irreps() == nirrep);
    TBLIS_ASSERT(C.num_irreps() == nirrep);
...
template void mult(const communicator& comm, \
                   T alpha, indexed_dpd_marray_view<const T> A, const label_type* idx_A, \
                            indexed_dpd_marray_view<const T> B, const label_type* idx_B, \
                   T  beta, indexed_dpd_marray_view<      T> C, const label_type* idx_C);
#include "configs/foreach_type.h"
