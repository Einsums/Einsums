...
REPLICATED_TEMPLATED_TEST_CASE(gemm, R, T, all_types)
{
    ...
    add<T>(T(-1), matrix_view<T const>(D), T(1), E);
    T error = reduce<T>(REDUCE_NORM_2, matrix_view<T const>(E)).first;
...
REPLICATED_TEMPLATED_TEST_CASE(gemm_diag, R, T, all_types)
{
    ...
    add<T>(T(-1), matrix_view<T const>(E), T(1), F);
    T error = reduce<T>(REDUCE_NORM_2, matrix_view<T const>(F)).first;
