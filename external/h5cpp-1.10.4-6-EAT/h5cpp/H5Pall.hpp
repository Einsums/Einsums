...
struct prop_base {
    ...
    template <class R>
    typename std::enable_if<std::is_same<typename R::hidtype, hidtype>::value, const prop_base<Derived, phid_t> &>::type
...
struct prop_t : prop_base<prop_t<phid_t, init, capi, capi_call>, phid_t> {
    ...
    prop_t(typename capi::args_t values) : args(values) {
        H5CPP_CHECK_NZ((this->handle = H5Pcreate(init())), h5::error::property_list::misc, "failed to create property");
    }
    prop_t() { H5CPP_CHECK_NZ((this->handle = H5Pcreate(init())), h5::error::property_list::misc, "failed to create property"); }
    void copy_impl(::hid_t id) const {
        // int i = capi_call + 1;
        /*CAPI needs `this` hid_t id passed along */
        capi_t capi_args = std::tuple_cat(std::tie(id), args);
        H5CPP_CHECK_NZ(compat::apply(capi_call, capi_args), h5::error::property_list::argument, "failed to parse arguments...");
    }
...
struct aprop_t final : prop_t<phid_t, init, capi, capi_call> {
    ...
    aprop_t(std::initializer_list<hsize_t> values) : base_t(std::make_tuple(values.size(), this->values)) {
        std::copy(std::begin(values), std::end(values), this->values);
    }
    template <class T, size_t N>
    aprop_t(std::array<T, N> values) : base_t(std::make_tuple(values.size(), this->values)) {
        std::copy(std::begin(values), std::end(values), this->values);
    }
...
H5CPP__capicall(acpl, H5P_ATTRIBUTE_CREATE) H5CPP__capicall(dapl, H5P_DATASET_ACCESS) H5CPP__capicall(dxpl, H5P_DATASET_XFER)
    H5CPP__capicall(dcpl, H5P_DATASET_CREATE) H5CPP__capicall(tapl, H5P_DATATYPE_ACCESS) H5CPP__capicall(tcpl, H5P_DATATYPE_CREATE)
        H5CPP__capicall(fapl, H5P_FILE_ACCESS) H5CPP__capicall(fcpl, H5P_FILE_CREATE) H5CPP__capicall(fmpl, H5P_FILE_MOUNT)

            H5CPP__capicall(gapl, H5P_GROUP_ACCESS) H5CPP__capicall(gcpl, H5P_GROUP_CREATE) H5CPP__capicall(lapl, H5P_LINK_ACCESS)
                H5CPP__capicall(lcpl, H5P_LINK_CREATE) H5CPP__capicall(ocpl, H5P_OBJECT_COPY) H5CPP__capicall(ocrl, H5P_OBJECT_CREATE)
...
using dcpl_tcall = tprop_t<h5::dcpl_t, default_dcpl, capi, capi_call, T>;
} // namespace impl
} // namespace h5
...
using file_space_page_strategy =
    impl::fcpl_call<impl::fcpl_args<hid_t, H5F_fspace_strategy_t, hbool_t, hsize_t>, H5Pset_file_space_strategy>;
...
using fapl_multi = impl::fapl_call<impl::fapl_args<hid_t, const H5FD_mem_t *, const hid_t *, const char *const *, const haddr_t *, hbool_t>,
                                   H5Pset_fapl_multi>;
...
// FIXME: using file_image_callback      = impl::fapl_call< impl::fapl_args<hid_t,H5_file_image_callbacks_t
// *callbacks_ptr>,H5Pset_file_image_callback>;
...
namespace h5 {
namespace notimplemented_yet { // OBJECT COPY PROPERTY LISTS
