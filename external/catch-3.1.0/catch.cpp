...
sample resample(URng &rng, unsigned int resamples, std::vector<double>::iterator first, std::vector<double>::iterator last,
                Estimator &estimator) {
...
bootstrap_analysis analyse_samples(double confidence_level, unsigned int n_resamples, std::vector<double>::iterator first,
...
#    pragma warning(pop)
    ...
    for (auto const &reporterSpec : m_data.reporterSpecifications) {
        ...
        m_processedReporterSpecs.push_back(
            ProcessedReporterSpec{reporterSpec.name(), reporterSpec.outputFile() ? *reporterSpec.outputFile() : data.defaultOutputFilename,
...
IEventListenerPtr prepareReporters(Config const *config) {
    ...
    for (auto const &reporterSpec : config->getProcessedReporterSpecs()) {
        multi->addReporter(createReporter(reporterSpec.name, ReporterConfig(config, makeStream(reporterSpec.outputFilename),
...
kvPair splitKVPair(StringRef kvString) {
    auto splitPos = static_cast<size_t>(std::distance(kvString.begin(), std::find(kvString.begin(), kvString.end(), '=')));
...
struct GeneratorTracker : TestCaseTracking::TrackerBase, IGeneratorTracker {
    ...
    static GeneratorTracker &acquire(TrackerContext &ctx, TestCaseTracking::NameAndLocation const &nameAndLocation) {
        ...
        if (currentTracker.nameAndLocation() == nameAndLocation) {
            ...
            auto newTracker = Catch::Detail::make_unique<GeneratorTracker>(nameAndLocation, ctx, &currentTracker);
    ...
    void close() override {
        ...
        const bool should_wait_for_child = [&]() {
            ...
            if (std::find_if(m_children.begin(), m_children.end(),
                             [](TestCaseTracking::ITrackerPtr const &tracker) { return tracker->hasStarted(); }) != m_children.end()) {
                return false;
            }
            ...
            for (auto const &child : m_children) {
                if (child->isSectionTracker() &&
                    std::find(filters.begin(), filters.end(), static_cast<SectionTracker const &>(*child).trimmedName()) != filters.end()) {
                    return true;
                }
            }
            return false;
...
RunContext::RunContext(IConfig const *_config, IEventListenerPtr &&reporter)
    : m_runInfo(_config->name()), m_context(getCurrentMutableContext()), m_config(_config),
      m_reporter(CATCH_MOVE(reporter)), m_lastAssertionInfo{StringRef(), SourceLineInfo("", 0), StringRef(), ResultDisposition::Normal},
...
void enforceNoDuplicateTestCases(std::vector<TestCaseHandle> const &tests) {
    ...
    for (auto const &test : tests) {
        ...
        CATCH_ENFORCE(prev.second, "error: test case \"" << infoPtr->name << "\", with tags \"" << infoPtr->tagsAsString()
                                                         << "\" already defined.\n"
...
ReporterBase::ReporterBase(ReporterConfig &&config)
    : IEventListener(config.fullConfig()), m_wrapped_stream(CATCH_MOVE(config).takeStream()), m_stream(m_wrapped_stream->stream()),
      m_colour(makeColourImpl(config.colourMode(), m_wrapped_stream.get())), m_customOptions(config.customOptions()) {
}
...
void printTotals(std::ostream &out, const Totals &totals, ColourImpl *colourImpl) {
    if (totals.testCases.total() == 0) {
        ...
        out << "Failed " << bothOrAll(totals.testCases.failed) << pluralise(totals.testCases.failed, "test case"_sr)
            << ", "
               "failed "
            << qualify_assertions_failed << pluralise(totals.assertions.failed, "assertion"_sr) << '.';
    } else if (totals.assertions.total() == 0) {
        out << "Passed " << bothOrAll(totals.testCases.total()) << pluralise(totals.testCases.total(), "test case"_sr)
            << " (no assertions).";
...
void CompactReporter::testRunStarting(TestRunInfo const &) {
    if (m_config->testSpec().hasFilters()) {
        m_stream << m_colour->guardColour(Colour::BrightYellow) << "Filters: " << serializeFilters(m_config->getTestsOrTags()) << '\n';
    }
...
void ConsoleReporter::lazyPrintRunInfo() {
    m_stream << '\n'
             << lineOfChars('~') << '\n'
             << m_colour->guardColour(Colour::SecondaryText) << currentTestRunInfo.name << " is a Catch2 v" << libraryVersion()
             << " host application.\n"
...
void ConsoleReporter::printSummaryRow(StringRef label, std::vector<SummaryColumn> const &cols, std::size_t row) {
    for (auto col : cols) {
        ...
        if (col.label.empty()) {
            ...
            m_stream << m_colour->guardColour(Colour::LightGrey) << " | " << m_colour->guardColour(col.colour) << value << ' ' << col.label;
        }
    }
    m_stream << '\n';
}
...
void XmlReporter::testCaseStarting(TestCaseInfo const &testInfo) {
    StreamingReporterBase::testCaseStarting(testInfo);
    m_xml.startElement("TestCase").writeAttribute("name"_sr, trim(testInfo.name)).writeAttribute("tags"_sr, testInfo.tagsAsString());
