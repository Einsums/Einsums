...
template<typename T, size_t OldN, size_t NewN, size_t... I>
constexpr inline std::array<std::remove_cv_t<T>, NewN> slice_array_impl(std::array<std::remove_cv_t<T>, OldN> const &old_array, std::index_sequence<I...> &) {
    return {{old_array[I]...}};
}

template<typename T, size_t OldN, size_t NewN, size_t... I>
constexpr inline std::array<std::remove_cv_t<T>, NewN> slice_array_impl(std::array<std::remove_cv_t<T>, OldN> &&old_array, std::index_sequence<I...> &) {
    return {{std::move(old_array[I])...}};
}

template<typename T, typename... TupleTypes, size_t... I>
requires(std::is_convertible_v<TupleTypes, T> && ...)
constexpr inline std::array<T, sizeof...(TupleTypes)> create_array_from_tuple_impl(std::tuple<TupleTypes...> const &tuple, std::index_sequence<I...> &) {
    return {{std::get<I>(tuple)...}};
}

template<typename T, typename... TupleTypes, size_t... I>
requires(std::is_convertible_v<TupleTypes, T> && ...)
constexpr inline std::array<T, sizeof...(TupleTypes)> create_array_from_tuple_impl(std::tuple<TupleTypes...> &&tuple, std::index_sequence<I...> &) {
    return {{std::move(std::get<I>(tuple))...}};
}

template<typename NewT, typename OldT, size_t N, size_t... I>
constexpr inline std::array<std::remove_cv_t<NewT>, N> convert_array_impl(std::array<std::remove_cv_t<OldT>, N> const &arr, std::index_sequence<I...> &) {
...
template<typename T, size_t OldN, size_t NewN>
    requires(OldN <= NewN)
constexpr std::array<std::remove_cv_t<T>, NewN> slice_array(std::array<std::remove_cv_t<T>, OldN> const &old_array) {
    return detail::slice_array_impl(old_array, std::make_index_sequence<NewN>());
}


template<typename T, size_t OldN, size_t NewN>
    requires(OldN <= NewN)
constexpr std::array<std::remove_cv_t<T>, NewN> slice_array(std::array<std::remove_cv_t<T>, OldN> &&old_array) {
    return detail::slice_array_impl(std::move(old_array), std::make_index_sequence<NewN>());
}
