...
void geev(AType *A, WType *W, AType *lvecs, AType *rvecs) {
    EINSUMS_ASSERT(A->dim(0) == A->dim(1)); EINSUMS_ASSERT(W->dim(0) == A->dim(0)); EINSUMS_ASSERT(A->dim(0) == lvecs->dim(0)); EINSUMS_ASSERT(A->dim(1) == lvecs->dim(1)); EINSUMS_ASSERT(A->dim(0) == rvecs->dim(0)); EINSUMS_ASSERT(A->dim(1) == rvecs->dim(1));
...
template <CoreBasicTensorConcept AType>
requires MatrixConcept<AType>
auto pow(AType const &a, typename AType::ValueType alpha, typename AType::ValueType cutoff =
        std::numeric_limits<typename AType::ValueType>::epsilon()) -> Tensor<typename AType::ValueType, 2> {
    assert(a.dim(0) == a.dim(1));
