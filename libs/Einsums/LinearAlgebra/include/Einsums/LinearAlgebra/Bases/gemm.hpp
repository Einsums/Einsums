...
template <typename T>
void impl_gemm_contiguous_no_conjugate(char transA, char transB, T alpha, einsums::detail::TensorImpl<T> const &A,
                                       einsums::detail::TensorImpl<T> const &B, T beta, einsums::detail::TensorImpl<T> *C) {
    char tA = std::tolower(transA), tB = std::tolower(transB);
    bool colA = A.is_column_major(), colB = B.is_column_major(), colC = C->is_column_major();
...
template <typename AType, typename BType, typename CType>
void impl_gemm_noncontiguous_no_conjugate(char transA, char transB, CType alpha, einsums::detail::TensorImpl<AType> const &A,
                                          einsums::detail::TensorImpl<BType> const &B, CType beta, einsums::detail::TensorImpl<CType> *C) {
    LabeledSection0();
...
template <typename AType, typename BType, typename CType, typename AlphaType, typename BetaType>
void impl_gemm_no_conjugate(char transA, char transB, AlphaType alpha, einsums::detail::TensorImpl<AType> const &A,
                            einsums::detail::TensorImpl<BType> const &B, BetaType beta, einsums::detail::TensorImpl<CType> *C) {
    if constexpr (!std::is_same_v<AType, BType> || !std::is_same_v<AType, CType>) {
        impl_gemm_noncontiguous_no_conjugate(transA, transB, einsums::detail::convert<CType>(alpha), A, B,
                                             einsums::detail::convert<CType>(beta), C);
    } else {
        if (A.is_gemmable() && B.is_gemmable() && C->is_gemmable()) {
            impl_gemm_contiguous_no_conjugate(transA, transB, einsums::detail::convert<AlphaType, CType>(alpha), A, B,
                                              einsums::detail::convert<AlphaType, CType>(beta), C);
        } else {
            impl_gemm_noncontiguous_no_conjugate(transA, transB, einsums::detail::convert<AlphaType, CType>(alpha), A, B,
                                                 einsums::detail::convert<AlphaType, CType>(beta), C);
        }
    }
}
