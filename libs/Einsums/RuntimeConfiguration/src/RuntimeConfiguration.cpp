...
std::vector<std::string> RuntimeConfiguration::parse_command_line(std::function<void()> const &user_command_line) {
    ...
    auto &global_bools   = global_config.get_bool_map()->get_value();
    {
        ...
        static cl::Opt<std::string> logFormat("einsums:log:format", {}, "Log format", logCategory,
        ...
        static cl::Opt<bool> noProfileAppend("einsums:profile:no-append", {}, "Don't append to profile file", profileCategory,
                                             cl::Location(global_bools["profiler-append"]), cl::Default(true), cl::ImplicitValue(false),
                                             cl::ValueName("N"));

        static cl::Flag profileDetailed("einsums:profile:detailed", {}, "Print detailed profile report", profileCategory,
