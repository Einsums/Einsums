...
struct Observable {
    ...
    // Assignment operator for setting the value
    Observable &operator=(T const &value) {
        set_value(value);
        return *this;
    }

    // Casting operator for retrieving the value
    operator T() const { return get_value(); }

    // Explicit set method (if direct assignment is not used)
    ...
    // Explicit get method (if direct casting is not used)
    T get_value() const {
        std::lock_guard<std::mutex> lock(_mutex);
        return _value;
    }
