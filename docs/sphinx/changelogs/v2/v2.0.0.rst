...
* Tensor storage order can now be selected by users. The default is still row-major, but this can be changed when you
  configure Einsums or can be passed as a command line option.
...
* Einsums has its own argument parser and can handle configuration files.
* `norm` can now handle vectors and can compute the spectral norm.
* Disk tensors can now be used in certain linear algebra routines. Currently working are `dot`, `true_dot`, `gemv`, `gemm`, `ger`,
  `gerc`, and `direct_prod`.  Work is being done to get `gesv` and `truncated_syev` working, though the implementation is not yet ready
* Tensors can be cached to GPU. When tensors are cached, certain operations can use GPU algorithms instead of CPU algorithms to speed up
  calculations.
* The `einsum` dispatcher now uses smarter rules for whether a tensor can be passed to a BLAS call.
* Some tensors can now specify an allocator template parameter which will be passed to the vector.
...
Bug Fixes
---------
* Fixed a bug where GPU kernels were being launched with the block and grid sizes in the wrong order, leading to 31 inactive threads
  in a 32-thread warp.
