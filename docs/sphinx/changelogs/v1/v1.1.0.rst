...
* The dispatching function for :cpp:func:`~einsums::einsum` is now more modular, so it is hopefully easier to maintain.
...
* ``sort`` is no longer a valid alias for :cpp:func:`~einsums::permute`.
