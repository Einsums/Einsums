...
BEGIN_EINSUMS_NAMESPACE_HPP(einsums::tensor_algebra::threads)
    ...
    if constexpr (sizeof...(CIndices) == 0 && sizeof...(LinkDims) != 0) {
        ...
        for (auto link_combination : std::apply(ranges::views::cartesian_product, link_dims)) {
            ...
            auto A_order = einsums::tensor_algebra::detail::construct_indices_from_unique_combination<AIndices...>(
                C_unique, {}, {}, link_unique, link_combination, link_position_in_link);
            auto B_order = einsums::tensor_algebra::detail::construct_indices_from_unique_combination<BIndices...>(
                C_unique, {}, {}, link_unique, link_combination, link_position_in_link);
...
void dot(const Type<T, 1> &A, const Type<T, 1> &B, int num_threads, int thread_id, T *work, std::atomic_int &synch) {
    if (thread_id != 0) {
        work[thread_id] = 0;
        return;
    }
    ...
    work[thread_id] = blas::dot(A.dim(0), A.data(), A.stride(0), B.data(), B.stride(0));
}

template <template <typename, size_t> typename Type, typename T, size_t Rank>
    requires CoreRankTensor<Type<T, Rank>, Rank, T>
void dot(const Type<T, Rank> &A, const Type<T, Rank> &B, int num_threads, int thread_id, T *work, std::atomic_int &synch) {
    ...
    dot(TensorView<T, 1>(const_cast<Type<T, Rank> &>(A), dim), TensorView<T, 1>(const_cast<Type<T, Rank> &>(B), dim), num_threads,
        thread_id, work);
}
...
void dot(const Type<T, Rank> &A, const Type<T, Rank> &B, const Type<T, Rank> &C, int num_threads, int thread_id, T *work,
    ...
    for (size_t i = thread_id; i < dim[0]; i += num_threads) {
        work[thread_id] += vA(i) * vB(i) * vC(i);
    }
...
auto einsum(const CDataType C_prefactor, const std::tuple<CIndices...> & /*Cs*/, CType<CDataType, CRank> *C,
            ...
            const BType<BDataType, BRank> &B, int num_threads, int thread_id,
            std::conditional_t<(sizeof(ADataType) > sizeof(BDataType)), ADataType, BDataType> *work, std::atomic_int &synch)
    ...
    constexpr auto C_exactly_matches_A =
        sizeof...(CIndices) == sizeof...(AIndices) && einsums::tensor_algebra::detail::same_indices<std::tuple<CIndices...>, std::tuple<AIndices...>>();
    constexpr auto C_exactly_matches_B =
        sizeof...(CIndices) == sizeof...(BIndices) && einsums::tensor_algebra::detail::same_indices<std::tuple<CIndices...>, std::tuple<BIndices...>>();
    constexpr auto A_exactly_matches_B = einsums::tensor_algebra::detail::same_indices<std::tuple<AIndices...>, std::tuple<BIndices...>>();
    ...
    if constexpr (!std::is_same_v<CDataType, ADataType> || !std::is_same_v<CDataType, BDataType>) {
        ...
        einsum_generic_algorithm(C_unique, A_unique, B_unique, link_unique, C_indices, A_indices, B_indices, unique_target_dims,
                                 ...
                                 num_threads, thread_id, work, synch);
        return;
    } else if constexpr (dot_product) {
        detail::dot(A, B, num_threads, thread_id, work, synch);

        (*C) *= C_prefactor;
        (*C) += AB_prefactor * work[0];
    ...
    einsum_generic_algorithm(C_unique, A_unique, B_unique, link_unique, C_indices, A_indices, B_indices, unique_target_dims,
                             unique_link_dims, target_position_in_C, link_position_in_link, C_prefactor, C, AB_prefactor, A, B, num_threads,
                             thread_id, work, synch);
}
...
auto einsum(const U UC_prefactor, const std::tuple<CIndices...> &C_indices, CType<CDataType, CRank> *C, const U UAB_prefactor,
    ...
    timer::push("testing");
    detail::einsum<true>(C_prefactor, C_indices, &testC, AB_prefactor, A_indices, A, B_indices, B, num_threads, thread_id, work, synch);
    timer::pop();
#endif
...
void einsum(const T C_prefactor, const std::tuple<CIndices...> &C_indices, CType *C, const T AB_prefactor,
            ...
            int num_threads, int thread_id, T *work, std::atomic_int &synch) {
    einsum(C_prefactor, C_indices, C, AB_prefactor, A_indices, A, B_indices, *B, num_threads, thread_id, work, synch);
}
...
auto einsum(const T C_prefactor, const std::tuple<CIndices...> &C_indices, CType *C, const T AB_prefactor,
            const std::tuple<AIndices...> &A_indices, const AType &A, const std::tuple<BIndices...> &B_indices, const BType &B,
            int num_threads, int thread_id, T *work, std::atomic_int &synch) {
    einsum(C_prefactor, C_indices, C->get(), AB_prefactor, A_indices, A, B_indices, B, num_threads, thread_id, work, synch);
}
...
void einsum(const std::tuple<CIndices...> &C_indices, CType *C, const std::tuple<AIndices...> &A_indices, const AType &A,
            const std::tuple<BIndices...> &B_indices, const BType &B, int num_threads, int thread_id, T *work, std::atomic_int &synch) {
    einsum(0, C_indices, C, 1, A_indices, A, B_indices, B, num_threads, thread_id, work, synch);
}
...
void einsum(const std::tuple<CIndices...> &C_indices, CType *C, const std::tuple<AIndices...> &A_indices, const AType &A,
            const std::tuple<BIndices...> &B_indices, const BType &B, int num_threads, int thread_id, T *work, std::atomic_int &synch) {
    einsum(0, C_indices, C->get(), 1, A_indices, A, B_indices, B, num_threads, thread_id, work, synch);
}
...
template <SmartPointer SmartPtr, typename UnaryOperator, typename T = double>
void element_transform(SmartPtr *C, UnaryOperator unary_opt, int num_threads, int thread_id, T *work, std::atomic_int &synch) {
    element_transform(C->get(), unary_opt, num_threads, thread_id, work, synch);
}
...
auto unfold(const CType<T, CRank> &source, int num_threads, int thread_id)
    ...
    auto source_position_in_source = einsums::tensor_algebra::detail::find_type_with_position(source_only, source_indices);
    auto link_position_in_source   = einsums::tensor_algebra::detail::find_type_with_position(link, source_indices);

    auto link_dims = einsums::tensor_algebra::detail::get_dim_ranges_for(
        target, einsums::tensor_algebra::detail::find_type_with_position(link, target_indices));
    auto source_dims = einsums::tensor_algebra::detail::get_dim_ranges_for(source, source_position_in_source);
    ...
    for (auto link_it = std::next(link_view.begin(), thread_id); link_it < link_view.end(); link_it = std::next(link_it, num_threads)) {
        size_t Z{0};
        for (auto source_it = source_view.begin(); source_it < source_view.end(); source_it++) {
            ...
            auto source_order = einsums::tensor_algebra::detail::construct_indices(source_indices, *source_it, source_position_in_source,
                                                                                   *link_it, link_position_in_source);
...
auto khatri_rao(const std::tuple<AIndices...> &, const AType<T, ARank> &A, const std::tuple<BIndices...> &, const BType<T, BRank> &B,
                int num_threads, int thread_id, T *work, std::atomic_int &synch, Tensor<T, 2> &out)
    ...
    out = Tensor<T, 2>{std::move(result), "KR product", -1, einsums::tensor_algebra::detail::product_dims(A_common_position, A)};
}

END_EINSUMS_NAMESPACE_HPP(einsums::tensor_algebra::threads)