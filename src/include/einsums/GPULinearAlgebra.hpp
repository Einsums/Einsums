...
void gemm(T alpha, const AType<T, Rank> &A, const BType<T, Rank> &B, T beta, CType<T, Rank> *C, hipStream_t stream = (hipStream_t) -1);
...
void dot(CDataType C_prefactor, CType<CDataType, 0> &C,
         ...
         const BType<BDataType, BRank> &B, hipStream_t stream = (hipStream_t) -1);
...
void ger(T alpha, const XYType<T, XYRank> &X, const XYType<T, XYRank> &Y, AType<T, ARank> *A, hipStream_t stream = (hipStream_t) -1);
...
void gemv(T alpha, const AType<T, ARank> &A, const XType<T, XYRank> &x, T beta, YType<T, XYRank> *y, hipStream_t stream = (hipStream_t) -1);
...
void scale(T scale, AType<T, ARank> *A, hipStream_t stream = (hipStream_t) -1);
...
void symm_gemm(const AType<T, Rank> &A, const BType<T, Rank> &B, CType<T, Rank> *C, hipStream_t stream = (hipStream_t) -1);
